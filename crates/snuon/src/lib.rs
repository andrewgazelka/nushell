...
mod tests {
    ...
    use crate::{ToStyle, from_snuon, to_snuon};

    /// test something of the form
    /// ```nushell
    /// $v | from snuon | to snuon | $in == $v
    /// ```
    ///
    /// an optional "middle" value can be given to test what the value is between `from snuon` and
    /// `to snuon`.
    fn snuon_end_to_end(input: &str, middle: Option<Value>) {
        let engine_state = EngineState::new();
        let val = from_snuon(input, None).unwrap();
        if let Some(m) = middle {
            assert_eq!(val, m);
        }
        assert_eq!(
            to_snuon(&engine_state, &val, ToStyle::Default, None, false).unwrap(),
            input
        );
    }
    ...
    #[test]
    fn from_snuon_illegal_table() {
        assert!(
            from_snuon("[[repeated repeated]; [abc, xyz], [def, ijk]]", None)
                .unwrap_err()
                .to_string()
    ...
    #[test]
    fn bool() {
        snuon_end_to_end("false", Some(Value::test_bool(false)));
    }

    #[test]
    fn raw_string_with_quotes() {
        // Test that strings with quotes use raw string syntax
        let engine_state = EngineState::new();
        let val = Value::test_string(r#"hello"world"#);
        let result = to_snuon(&engine_state, &val, ToStyle::Default, None, false).unwrap();
        assert_eq!(result, r#"r#'hello"world'#"#);

        // Test round-trip
        let parsed = from_snuon(&result, None).unwrap();
        assert_eq!(parsed, val);
    }

    #[test]
    fn raw_string_with_backslash() {
        // Test that strings with backslashes use raw string syntax
        let engine_state = EngineState::new();
        let val = Value::test_string(r"hello\world");
        let result = to_snuon(&engine_state, &val, ToStyle::Default, None, false).unwrap();
        assert_eq!(result, r#"r#'hello\world'#"#);
    ...
    fn raw_string_in_list() {
        // Test raw strings in lists
        let engine_state = EngineState::new();
        let val = Value::test_list(vec![Value::test_string(r#"hello"world"#)]);
        let result = to_snuon(&engine_state, &val, ToStyle::Default, None, false).unwrap();
        assert_eq!(result, r#"[r#'hello"world'#]"#);
    ...
    #[test]
    fn raw_string_in_record() {
        // Test raw strings in records
        let engine_state = EngineState::new();
        let val = Value::test_record(record!(
            "s" => Value::test_string(r#"hello"world"#)
        ));
        let result = to_snuon(&engine_state, &val, ToStyle::Default, None, false).unwrap();
        assert_eq!(result, r#"{s: r#'hello"world'#}"#);
    ...
    #[test]
    fn raw_string_nested() {
        // Test nested raw strings (strings containing raw string syntax)
        // Since this string doesn't contain " or \, it will use regular quotes
        let engine_state = EngineState::new();
        let val = Value::test_string("r#'test'#");
        let result = to_snuon(&engine_state, &val, ToStyle::Default, None, false).unwrap();
        // Should use regular double quotes since no escaping is needed
        assert_eq!(result, r#""r#'test'#""#);
    ...
    #[test]
    fn raw_string_complex() {
        // Test a string that contains both quotes and raw string syntax
        let engine_state = EngineState::new();
        let val = Value::test_string(r#"He said "hello" to me"#);
        let result = to_snuon(&engine_state, &val, ToStyle::Default, None, false).unwrap();
        // Should use raw strings with # to avoid escaping the quotes
        assert_eq!(result, r##"r#'He said "hello" to me'#"##);
    ...
    #[test]
    fn negative_int() {
        snuon_end_to_end("-1", Some(Value::test_int(-1)));
    }
    ...
    #[test]
    fn filesize() {
        snuon_end_to_end("1024b", Some(Value::test_filesize(1024)));
        assert_eq!(from_snuon("1kib", None).unwrap(), Value::test_filesize(1024));
    }

    #[test]
    fn duration() {
        snuon_end_to_end("60000000000ns", Some(Value::test_duration(60_000_000_000)));
    }

    #[test]
    fn to_snuon_datetime() {
        snuon_end_to_end(
    ...
    fn binary_roundtrip() {
        ...
        assert_eq!(
            to_snuon(
                &engine_state,
                &from_snuon("0x[1f ff]", None).unwrap(),
                ToStyle::Default,
                None,
                false,
            )
    ...
    #[test]
    fn to_snuon_converts_columns_with_spaces() {
        let engine_state = EngineState::new();
    ...
    #[test]
    fn to_snuon_quotes_empty_string() {
        let engine_state = EngineState::new();
    ...
    #[test]
    fn to_snuon_quotes_empty_string_in_list() {
        snuon_end_to_end(
            r#"[""]"#,
    ...
    #[test]
    fn to_snuon_quotes_empty_string_in_table() {
        snuon_end_to_end(
            "[[a, b]; [\"\", la], [le, lu]]",
            Some(Value::test_list(vec![
                Value::test_record(record!(
    ...
    #[test]
    // NOTE: this test could be stronger, but the output of [`from_snuon`] on the content of `../../../tests/fixtures/formats/code.nu` is
    ...
    //     msg: "calls not supported in snuon",
