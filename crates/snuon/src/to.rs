...
///
/// **WARNING**: This is "Stupid NUON" - it does NOT escape strings and cannot be parsed back!
/// Do not use this for round-trip serialization.
///
/// > **Note**
/// > a [`Span`] can be passed to [`to_snuon`] if there is context available to the caller
pub fn to_snuon(
...
fn value_to_string(
    ...
    match v {
        ...
        Value::Closure { val, .. } => {
            if serialize_types {
                // SNUON: No escaping, just raw string
        ...
        Value::List { vals, .. } => {
            ...
            if !headers.is_empty() && vals.iter().all(|x| x.columns().eq(headers.iter())) {
                ...
                let headers: Vec<String> = headers
                    .iter()
                    .map(|string| {
                        // SNUON: No escaping, always use raw string
                        format!("{idt}{string}")
                    })
                    .collect();
        ...
        Value::Record { val, .. } => {
            let mut collection = vec![];
            for (col, val) in &**val {
                // SNUON: No escaping, always use raw column name
                collection.push(format!(
                    "{idt_po}{col}:{kv_sep}{}",
                    value_to_string_without_quotes(
                        engine_state,
        ...
        // SNUON: No escaping at all, just return raw strings
        Value::String { val, .. } => Ok(val.clone()),
        Value::Glob { val, .. } => Ok(val.clone()),
    }
}
...
fn value_to_string_without_quotes(
    ...
    match v {
        // SNUON: No escaping or quoting checks, just return raw string
        Value::String { val, .. } => Ok(val.clone()),
